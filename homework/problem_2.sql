...
SELECT ename,SUBSTR(job,2,4) FROM EMP;
...
select ename,sal/7,round(sal/7),trunc(sal/7) from emp;
...
from emp;
--count(columns)  --컬럼명을 집어넣으면은 null인것들은 제외가 됨
-----------  Home Work   Group 함수   --------------------
...
select deptno,grade,count(*),round(avg(weight),0)
from student
